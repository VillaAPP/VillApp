...
public class CargarOpiniones extends AsyncTask<String, Void, ArrayList<Opinion>> {
    ...
    public CargarOpiniones(IOpiniones iOpiniones){
        this.iOpiniones = iOpiniones;
    }

    protected ArrayList<Opinion> doInBackground(String... params) {
        ArrayList<Opinion> lista = new ArrayList<Opinion>();

        try {
            URL url = new URL("http://www.villaApp.esy.es/get_opiniones.php");
                ...
                if(estadoJSON.equals("1")){
                    ...
                    if(!auxJSON.equals("[]")){
                        ...
                        for(int i = 0; i < opinionesJSON.length(); i++){
                            ...
                            Opinion opinion = new Opinion(opinionJSON.getString("userName"), opinionJSON.getInt("rate"), opinionJSON.getString("opinion"));
                            lista.add(opinion);
                        }
                    }
                }
